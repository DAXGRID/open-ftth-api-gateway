    ...
    public class NodeContainerBuilder
    {
        ...
        private void AffixPassThroughConduit(LineBlock nodeContainerBlock, SpanEquipmentViewModel viewModel)
        {
            ...
            foreach (var innerSpan in innerSpanData)
            {
                ...
                if (innerSpan.IsPassThrough)
                {
                    fromNodeName = viewModel.GetFromRouteNodeName(innerSpan.SegmentId, null);
                    toNodeName = viewModel.GetToRouteNodeName(innerSpan.SegmentId, null);
                }
                else
                {
                    fromNodeName = viewModel.GetFromRouteNodeName(innerSpan.IngoingSpanSegment.Id, null);
                    toNodeName = viewModel.GetToRouteNodeName(innerSpan.OutgoingSpanSegment.Id, null);
                }
